...
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for xfce4
...
#SERVICE_USER="${SERVICE_USER:-xfce4}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-xfce4}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for xfce4"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
